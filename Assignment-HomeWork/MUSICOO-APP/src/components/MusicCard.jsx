...
function MusicCard({ singleSong, onPlay }) {
  const { favsongs, addFavSong, removeFavSong, theme } = Store();

  const isfav = favsongs.some(
    (fav) => fav.trackId === singleSong.trackId
  );

  return (
    <div
      className={`group flex h-\[520px] w-\[340px] flex-col rounded-2xl border p-6 shadow-lg transition-all duration-300 hover:scale-[1.03] hover:shadow-2xl ${
        theme === "light"
    ...
    >
      ...
      <div className="mt-5 flex flex-1 flex-col items-center">
        <h2
          className={`h-14 w-full overflow-hidden text-center text-xl font-bold line-clamp-2 ${
            theme === "light"
              ? "text-slate-900"
              : "text-white"
          }`}
        ...
        <p
          className={`mt-2 h-6 w-full truncate text-center text-base ${
            theme === "light"
              ? "text-slate-500"
              : "text-gray-300"
          }`}
